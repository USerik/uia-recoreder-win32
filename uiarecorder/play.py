...
def generate_snippet(generated_path):
    ...
    if generated_path:
        ...
        for item_path in dropwhile(lambda x: int(x[0]) > 1 or int(x[0]) == top_index,
                                   ordered_path.items()):
            generated_code += ".descendants(class_name='{0}', title='{1}',"\
                "control_type='{2}', depth={4})[{3}]".format(
                    ...
                    0 if int(
                        item_path[0]) == 0 else item_path[1]['top_parent_index'],
                    top_index -
...
def get_json_file(filename, elem_dir='recorder_elems'):
    ...
    if not pathlib.Path(elem_dir_).is_dir():
        ...
        return
    elem_file = pathlib.Path.joinpath(
        current_dir, elem_dir, f'{filename}.json')

    if not pathlib.Path.is_file(elem_file):
        print(
            f'Please, pass the correct filename. File does not exist: {filename}')
        return
